-- 0020_prestige_rank_workspace_revenue_exec.sql
-- Reps no longer get a sales leaderboard (that's an exec/owner tool). Retune the
-- two sensitivity levels on rep_scoreboard:
--   * prestige_rank (just the tier badge, 0-7): now workspace-wide for ALL roles
--     when show_prestige is on, so the rank PILL can appear next to anyone's name
--     (profile, squad, the performance leaderboard). Coarse band, owner-opt-in.
--   * revenue / revenue_month (the actual dollars): restricted to OWNER + EXEC
--     only (still gated by show_prestige). Reps + managers can no longer read
--     colleagues' revenue figures — reps focus on their own numbers.
--   * pillar sub-scores (book/rel/activity): unchanged (own-team only).
-- Cross-tenant still sealed by the workspace WHERE.
...
with agg as (
  ...
  left join lateral (
    select count(*) as act_count from public.activities a2
...
select
  ...
  -- pillar sub-scores: own-team only (rep squad breakdown)
  ...
  -- prestige RANK (badge only): workspace-wide when prestige is on
  case when c.show_prestige then pl.rank_idx end as prestige_rank,
  -- revenue DOLLARS: owner + executive only
