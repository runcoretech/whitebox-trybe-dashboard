-- 0018_rep_scoreboard_revenue.sql
-- Expose per-rep REVENUE for the Sales Revenue Leaderboard. Unlike the pillar
-- sub-scores + prestige_rank (own-team only), the leaderboard is a company-wide
-- sales board, so `revenue` (lifetime) and `revenue_month` (current month) are
-- exposed for EVERY rep in the workspace — but ONLY when the owner has prestige
-- switched ON (show_prestige). Cross-tenant is still sealed: the view's WHERE
-- limits rows to the caller's own workspace (get_my_workspace()), so other
-- companies' reps never appear. Off → both columns NULL (no leaderboard).
--
-- Only change vs 0017: the orders lateral also sums the current month, and the
-- final select adds revenue / revenue_month (show_prestige-gated, workspace-wide).
...
pillars as (
  select
    id, name, manager_id, rev, rev_month,
...
select
  ...
  -- Company-wide revenue for the leaderboard (dollars visible), but ONLY when the
  -- owner has prestige on. Cross-tenant still sealed by the workspace WHERE above.
  case when c.show_prestige then pl.rev::numeric        end as revenue,
  case when c.show_prestige then pl.rev_month::numeric  end as revenue_month
from pillars pl
cross join caller c;
